...
class AccountBankStatementLine(models.Model):
    ...
    def action_save_new(self):
        action = self.env['ir.actions.act_window']._for_xml_id('sdm_accounting.action_bank_statement_line_form_bank_rec_widget')
    ...
    def _action_open_bank_reconciliation_widget(self, extra_domain=None, default_context=None, name=None, kanban_first=True):
        ...
        views = [
            (self.env.ref('sdm_accounting.view_bank_statement_line_kanban_bank_rec_widget').id, 'kanban'),
            (self.env.ref('sdm_accounting.view_bank_statement_line_tree_bank_rec_widget').id, 'list'),
        ]
        ...
        return {
            ...
            'context': context,
            'search_view_id': [self.env.ref('sdm_accounting.view_bank_statement_line_search_bank_rec_widget').id, 'search'],
    ...
    def _cron_try_auto_reconcile_statement_lines(self, batch_size=None, limit_time=0):
        ...
        if remaining_line_id:
            ...
            if nb_auto_reconciled_lines or not remaining_st_line.cron_last_check:
                self.env.ref('sdm_accounting.auto_reconcile_bank_statement_line')._trigger()
