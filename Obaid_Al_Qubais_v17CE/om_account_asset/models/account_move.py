...
class AccountMove(models.Model):
    ...
    asset_ids = fields.One2many('account.asset.asset', 'invoice_id',
                                string="Assets")
    assets_count = fields.Integer(string='Assets Count', compute='_compute_assets_count')
    ...
    def _compute_assets_count(self):
        for record in self:
            record.assets_count = len(record.asset_ids)

    def action_view_accounting_assets(self):
        self.ensure_one()
        return {
            'name': _('Assets'),
            'view_mode': 'tree,form',
...
class AccountMoveLine(models.Model):
    ...
    def asset_create(self):
        if self.asset_category_id:
            ...
            for i in range(qty):
                price_subtotal = self.currency_id._convert(
                    self.price_subtotal / qty,
                    self.company_currency_id,
                    self.company_id,
                    self.move_id.invoice_date or fields.Date.context_today(self)
                )
                vals = {
                    'name': f"{self.name} #{i + 1}",
                    'code': f"{self.name or ''}-{i + 1}",
