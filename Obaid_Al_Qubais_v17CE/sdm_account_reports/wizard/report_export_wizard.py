...
class ReportExportWizard(models.TransientModel):
    ...
    """
    _name = 'sdm_account_reports.export.wizard'
    _description = "Export wizard for accounting's reports"

    export_format_ids = fields.Many2many(string="Export to", comodel_name='sdm_account_reports.export.wizard.format', relation="dms_acc_rep_export_wizard_format_rel")
        ...
        for wizard in wizards:
            ...
            for button_dict in self._context.get('account_report_generation_options', {}).get('buttons', []):
                if button_dict.get('file_export_type'):
                    self.env['sdm_account_reports.export.wizard.format'].create({
...
class ReportExportWizardOption(models.TransientModel):
    _name = 'sdm_account_reports.export.wizard.format'
    ...
    export_wizard_id = fields.Many2one(string="Parent Wizard", comodel_name='sdm_account_reports.export.wizard', required=True, ondelete='cascade')
