...
class MulticurrencyRevaluationReportCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        return {
            'components': {
                'AccountReportFilters': 'sdm_account_reports.MulticurrencyRevaluationReportFilters',
            },
            'templates': {
                'AccountReportLineName': 'sdm_account_reports.MulticurrencyRevaluationReportLineName',
            },
        }
    ...
    def _custom_line_postprocessor(self, report, options, lines, warnings=None):
        line_to_adjust_id = self.env.ref('sdm_account_reports.multicurrency_revaluation_to_adjust').id
        line_excluded_id = self.env.ref('sdm_account_reports.multicurrency_revaluation_excluded').id
        ...
        if warnings is not None:
            if len(self.env.companies) > 1:
                warnings['sdm_account_reports.multi_currency_revaluation_report_warning_multicompany'] = {'alert_type': 'warning'}
            if options['custom_rate']:
                warnings['sdm_account_reports.multi_currency_revaluation_report_warning_custom_rate'] = {'alert_type': 'warning'}
    ...
    def action_multi_currency_revaluation_open_revaluation_wizard(self, options):
        ...
        form = self.env.ref('sdm_account_reports.view_account_multicurrency_revaluation_wizard', False)
        return {
    ...
    def action_multi_currency_revaluation_open_general_ledger(self, options, params):
        ...
        general_ledger_options = self.env.ref('sdm_account_reports.general_ledger_report').get_options(options)
        general_ledger_options['unfolded_lines'] = [account_line_id]

        general_ledger_action = self.env['ir.actions.actions']._for_xml_id('sdm_account_reports.action_account_report_general_ledger')
