...
class AccountReport(models.Model):
    ...
    def open_report_export_wizard(self, options):
        ...
        view_id = self.env.ref('sdm_account_reports.view_report_export_wizard').id


        new_wizard = self.with_context(new_context).env['sdm_account_reports.export.wizard'].create({'report_id': self.id})

        return {
            'type': 'ir.actions.act_window',
            'name': _('Export'),
            'view_mode': 'form',
            'res_model': 'sdm_account_reports.export.wizard',
    ...
    def caret_option_open_general_ledger(self, options, params):
        ...
        general_ledger = self.env.ref('sdm_account_reports.general_ledger_report')
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('sdm_account_reports.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': gl_options,
            'ignore_session': True,
        }
    ...
    def _generate_common_warnings(self, options, warnings):
        if options.get('available_tax_units') and options['tax_unit'] == 'company_only':
            warnings['sdm_account_reports.common_warning_tax_unit'] = {}

        if options.get('date') and options.get('all_entries') is not None:
            date_to = options['date'].get('date_to') or options['date'].get('date') or fields.Date.today()
            period_domain = [('state', '=', 'draft'), ('date', '<=', date_to)]
            if self.env['account.move'].search_count(period_domain):
                warnings['sdm_account_reports.common_warning_draft_in_period'] = {}
    ...
    def _compute_totals_no_batch_aggregation(self, column_group_options, formulas_dict, other_current_report_expr_totals, other_cross_report_expr_totals_by_scope):
        ...
        for (formula, forced_date_scope), expressions in formulas_dict.items():
            for expression in expressions:
                aggregations_terms_to_evaluate.add(f"_expression:{expression.id}")
    ...
    def action_display_inactive_sections(self, options):
        ...
        return {
            ...
            'views': [(False, 'list'), (False, 'form')],
            'context': {
                'tree_view_ref': 'sdm_account_reports.account_report_add_sections_tree',
                'active_test': False,
            },
        }
    ...
    def sort_lines(self, lines, options, result_as_index=False):
        ...
        descending = options['order_column']['direction'] == 'DESC'
    ...
    def _report_expand_unfoldable_line_groupby_prefix_group(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data=None):
        ...
        for markup, model, model_id in reversed(self._parse_line_id(line_dict_id)):
            if model == 'account.report.line':
                report_line_id = model_id
                break
            elif markup.startswith('groupby'):
    ...
    def export_to_pdf(self, options):
        ...
        footer = self.env['ir.actions.report']._render_template("sdm_account_reports.internal_layout", values=rcontext)
    ...
    def _get_pdf_export_html(self, options, lines, additional_context=None, template=None):
        ...
        template = custom_print_templates.get('pdf_export_main', 'sdm_account_reports.pdf_export_main')
    ...
    def _compute_is_account_coverage_report_available(self):
        for report in self:
            report.is_account_coverage_report_available = (
                ...
                and
                self.root_report_id in (self.env.ref('sdm_account_reports.profit_and_loss'), self.env.ref('sdm_account_reports.balance_sheet'))
            )
    ...
    def _generate_accounts_coverage_report_xlsx_lines(self):
        ...
        all_reported_accounts = self.env["account.account"]
        accounts_by_expressions = {}
        reported_account_codes = []
        non_existing_codes = defaultdict(lambda: self.env["account.report.line"])
        ...
        candidate_duplicate_codes = defaultdict(lambda: self.env["account.report.line"])
        duplicate_codes = defaultdict(lambda: self.env["account.report.line"])
        duplicate_codes_same_line = defaultdict(lambda: self.env["account.report.line"])
        ...
        if self.root_report_id == self.env.ref('sdm_account_reports.profit_and_loss'):
