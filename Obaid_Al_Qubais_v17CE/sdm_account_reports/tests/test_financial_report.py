...
class TestFinancialReport(TestAccountReportsCommon):
    ...
    def test_financial_report_single_company(self):
        line_id = self._get_basic_line_dict_id_from_report_line_ref('sdm_account_reports.account_financial_report_bank_view0')
        self.report.filter_multi_company = 'disabled'
    ...
    def test_financial_report_comparison_multi_company_currency_multi_rates(self):
        ...
        other_company_move_2021 = self.env['account.move'].with_company(other_company_data['company']).create({
            ...
            'line_ids': [
                ...
                (0, 0, {'debit': 0.0,    'credit': 1500.0, 'account_id': other_company_data['default_sdm_asset_managements'].id,     'partner_id': partner.id}),
            ],
        })
    ...
    def test_financial_report_horizontal_group(self):
        line_id = self._get_basic_line_dict_id_from_report_line_ref('sdm_account_reports.account_financial_report_receivable0')
    ...
    def test_option_hierarchy(self):
        ...
        move.action_post()
        move.line_ids.flush_recordset()
        profit_and_loss_report = self.env.ref('sdm_account_reports.profit_and_loss')
        line_id = self._get_basic_line_dict_id_from_report_line_ref('sdm_account_reports.account_financial_report_income0')
