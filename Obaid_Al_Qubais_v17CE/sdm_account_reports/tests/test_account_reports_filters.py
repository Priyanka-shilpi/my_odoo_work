...
class TestAccountReportsFilters(TestAccountReportsCommon, odoo.tests.HttpCase):
    ...
    def test_filter_aml_ir_filters(self):
        ...
        report = self.env['account.report'].create({
            ...
            'root_report_id': self.env.ref("sdm_account_reports.profit_and_loss").id,
            'column_ids': [
                Command.create({
                    'name': 'Balance',
    ...
    def test_hide_line_at_0_tour(self):
        report = self.env.ref('sdm_account_reports.balance_sheet')
        ...
        self.start_tour("/web", 'sdm_account_reports_hide_0_lines', login=self.env.user.login)
